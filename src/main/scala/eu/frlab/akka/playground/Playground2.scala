...
object Playground2 extends App {
  ...
  val graph: Graph[SourceShape[String], NotUsed] = GraphDSL.create() { implicit b =>
    ...
    bcast.out(0) ~> Flow[Outer].map(_.innerX).mapConcat(i => i.map(ii => s"${ii.i}${ii.b}")) ~> merge.in(0)
    bcast.out(1) ~> Flow[Outer].map(_.innerY).mapConcat(i => i.map(ii => s"${ii.i}${ii.b}")) ~> merge.in(1)
