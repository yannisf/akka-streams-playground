...
object Playground3b extends App {
  ...
  val graph =
    GraphDSL.create() { implicit builder => // builder = MUTABLE data structure
  ...
  val eventualDone: NotUsed = RunnableGraph.fromGraph(graph).run()
